package bg.aspar.birt.designer;
...
public class StaticJavaReportDesignApplication implements CommandLineRunner {

	private static final Logger log = Logger.getLogger(StaticJavaReportDesignApplication.class);

	@Value("${reports.relative.path}")
	private String REPORTS_FOLDER;

	public static void main(String[] args) {
		new SpringApplicationBuilder(StaticJavaReportDesignApplication.class).web(WebApplicationType.NONE).build().run(args);
	}
	...
	private void buildReport() throws IOException, BirtException {
		...
		label.setText("Hello, Bulgaria!");

		// Save the design and close it.
		File report = new File(REPORTS_FOLDER);
		report.mkdirs();

		design.saveAs(new File(report, "static-generated.rptdesign").getAbsolutePath());
		design.close();
